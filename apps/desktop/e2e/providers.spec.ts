...
test('provider connections: header geometry, the canonical API-key journey, and two-field rows', async ({ window: page }) => {
  // One window, three phases over one connection list. Phase 1 ends with the
  // seeded connection deleted (an empty list), which is exactly the state the
  // add-journey needs; the add-journey deletes what it created, which is the
  // state the two-field relay phase needs.
  ...
  await test.step('the catalog reaches Cerebras and renders its color brand asset untouched', async () => {
    // Already on the models page — phase 1 landed here after its reload.
